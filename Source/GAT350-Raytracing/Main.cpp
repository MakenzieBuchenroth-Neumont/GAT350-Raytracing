...
void InitScene01(Scene& scene, const Canvas& canvas)
{
	...
	for (int x = -10; x < 10; x++)
	{
		for (int z = -10; z < 10; z++)
		{
			...
			float radius = random(0.2f, 0.3f);
			// create sphere using random radius and material
			auto sphere = std::make_unique<Sphere>(glm::vec3{ x + random(-0.5f, 0.5f), 1, z + random(-0.5f, 0.5f) }, radius, material);
			// add sphere to the scene
			scene.AddObject(std::move(sphere));
		}
	}
...
void InitCornellBox(Scene& scene, const Canvas& canvas) {
	float aspectRatio = canvas.getSize().x / canvas.getSize().y;
	std::shared_ptr<Camera> camera = std::make_shared<Camera>(glm::vec3{ 0, 2, 5 }, glm::vec3{ 0, 1, 0 }, glm::vec3{ 0, 1, 0 }, 60.0f, aspectRatio);
	scene.SetCamera(camera);

	auto mesh = std::make_unique<Mesh>(std::make_shared<Lambertian>(color3_t{ 1, 1, 1 }));
	mesh->Load("models/quad.obj", glm::vec3{ 0, 1, -3 }, glm::vec3{ 0, 0, 0 }, glm::vec3{ 4.0f });
	scene.AddObject(std::move(mesh));

	mesh = std::make_unique<Mesh>(std::make_shared<Lambertian>(color3_t{ 1, 0, 0 }));
	mesh->Load("models/quad.obj", glm::vec3{ -2, 1, -1 }, glm::vec3{ 0, 90, 0 }, glm::vec3{ 4.0f });
	scene.AddObject(std::move(mesh));

	mesh = std::make_unique<Mesh>(std::make_shared<Lambertian>(color3_t{ 0, 1, 0 }));
	mesh->Load("models/quad.obj", glm::vec3{ 2, 1, -1 }, glm::vec3{ 0, 270, 0 }, glm::vec3{ 4.0f });
	scene.AddObject(std::move(mesh));

	mesh = std::make_unique<Mesh>(std::make_shared<Lambertian>(color3_t{ 1, 1, 1 }));
	mesh->Load("models/quad.obj", glm::vec3{ 0, -1, -1 }, glm::vec3{ 270, 0, 0 }, glm::vec3{ 4.0f });
	scene.AddObject(std::move(mesh));

	mesh = std::make_unique<Mesh>(std::make_shared<Lambertian>(color3_t{ 1, 1, 1 }));
	mesh->Load("models/quad.obj", glm::vec3{ 0, 3, -1 }, glm::vec3{ 90, 0, 0 }, glm::vec3{ 4.0f });
	scene.AddObject(std::move(mesh));

	mesh = std::make_unique<Mesh>(std::make_shared<Emissive>(glm::rgbColor(glm::vec3{ 0, 0, 1 }), 5.0f));
	mesh->Load("models/cube.obj", glm::vec3{ 0, 2.7, -1 }, glm::vec3{ 90, 0, 0 }, glm::vec3{ 1.0f });
	scene.AddObject(std::move(mesh));

	mesh = std::make_unique<Mesh>(std::make_shared<Dielectric>(color3_t{1,1,1}, 1.5f));
	mesh->Load("models/cube.obj", glm::vec3{ -1, 0, 0 }, glm::vec3{ 90,0,0 }, glm::vec3{ 1, 2, 1 });
	scene.AddObject(std::move(mesh));

	auto sphere = std::make_unique<Sphere>(glm::vec3{ 1,-0.5,0 }, 0.5, std::make_shared<Metal>(color3_t{ 1,1,1 }, 0));
	scene.AddObject(std::move(sphere));
}
...
void InitScene03(Scene& scene, const Canvas& canvas) {
	...
	for (int x = 0; x < 10; x++)
	{
		for (int z = 0; z < 10; z++)
		{
			...
			// create sphere using random radius and material
			//auto sphere = std::make_unique<Sphere>(glm::vec3{ x + random(-0.5f, 0.5f), radius, z + random(-0.5f, 0.5f) }, radius, material);
			auto sphere = std::make_unique<Sphere>(glm::vec3{ random(-5, 5), 1, random(0,10) }, radius, material);
			// add sphere to the scene
			scene.AddObject(std::move(sphere));
		}
	}
	...
	auto plane = std::make_unique<Plane>(glm::vec3{ 0, 0, 0 }, glm::vec3{ 0, 0, 0 }, std::make_shared<Lambertian>(color3_t{ 0.2f }));
	scene.AddObject(std::move(plane));
}